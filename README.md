...
- `JPP-SSH`：连接本机 `127.0.0.1:1089` 的无认证 SOCKS5 隧道；
- `DIRECT`：保留为最后一个手动选项。
...
默认规格已包含这条记录和同形态的 `JPP-SSH`（`127.0.0.1:1089`），不要重复添加。其他自定义
节点使用不同名称即可；生成器按列表顺序
...
生成器不启动或检查隧道。选择 `HKP-SSH` 或 `JPP-SSH` 前，必须确认对应本机端口（`1088`、
`1089`）能实际代理请求；仅看到 `autossh` 进程不够。`127.0.0.1` 指的是运行 Clash 的设备，
同步到手机不能借此访问 Mac。默认规格的 SSH 节点不可用时，仍可选择其他节点；手动选择组
不会自动回退。

现有路由规则不变；若在当前选择组中选中某个 SSH 节点，兜底流量也会走该出口。`HKP-SSH`
（香港）实测被 OpenAI API 以地区不支持拒绝，因此不要把它当作 OpenAI 出口；`JPP-SSH`
（日本，实测出口东京）对 OpenAI API 返回正常认证错误而非地区拒绝，可作为 OpenAI 出口。
需要按网站固定分流时，仍在下述规则模板中将 Google/YouTube 指向 `HKP-SSH`，将 OpenAI
指向 `JPP-SSH` 或其他已验证可用的节点；
